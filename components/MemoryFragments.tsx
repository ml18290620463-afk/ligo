...
export const MemoryFragments: React.FC<MemoryFragmentsProps> = ({ onComplete, language, principles }) => {
  ...
  const getYearStyle = (year: number) => {
    switch (year) {
      case 2023: return "text-cyan-200";
      case 2024: return "text-blue-200";
      case 2025: return "text-indigo-200";
  ...
  const getYearGlow = (year: number) => {
    switch (year) {
      case 2023: return "text-cyan-950/30";
      case 2024: return "text-blue-950/30";
      case 2025: return "text-indigo-950/30";
  ...
  return (
    <div className="relative min-h-screen bg-black overflow-hidden flex flex-col items-center justify-center perspective-[1000px]">
      ...
      <div className="absolute inset-0 pointer-events-none">
         ...
         <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.02)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.02)_1px,transparent_1px)] bg-[size:150px_150px] [transform:perspective(500px)_rotateX(60deg)_scale(3)] animate-[tunnel_40s_linear_infinite] opacity-20"></div>
      </div>

      {/* Central Year */}
      <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-0 font-black text-[20vw] leading-none transition-all duration-1000 select-none blur-[2px] ${getYearGlow(currentYear)}`}>
        {currentYear}
      </div>
            ...
            >
              ...
              <div className={!isExiting && isVisible ? 'animate-twinkle' : ''}>
                <div className="flex flex-col items-center gap-2">
                   ...
                   <Sparkles className={`w-3 h-3 opacity-50 ${getYearStyle(frag.year).split(' ')[0]} ${isVisible ? 'animate-pulse' : ''}`} />
              ...
              <div className={`mt-2 flex gap-3 text-[9px] font-mono tracking-[0.3em] uppercase transition-opacity duration-1000 delay-500 ${isVisible ? 'opacity-40' : 'opacity-0'} ${isExiting ? '!opacity-0' : ''} text-gray-500`}>
                 <span>{frag.year}</span>
                 <span>//</span>
      ...
      {/* Controls */}
      <div className={`relative z-50 transition-all duration-1000 flex flex-col items-center gap-6 mt-40 ${showButton ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
