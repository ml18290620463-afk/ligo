    ...
    >
      ...
      <div className="flex flex-col xl:flex-row gap-6">
        <div className="flex flex-1 gap-5 items-start">
          <div
            className={`w-20 h-20 rounded-full flex items-center justify-center border shrink-0 shadow-inner ${theme === 'light' ? 'bg-[#f0f9fa] border-cyan-100' : 'bg-cyan-950/20 border-cyan-900/30'}`}
          >
            <Sailboat
          ...
          <div className="flex-1 min-w-0 py-1">
            <div className="flex flex-wrap items-baseline gap-2 mb-1">
              ...
              />
              <span
                className={`text-xs font-mono opacity-60 ${theme === 'light' ? 'text-slate-500' : 'text-[#6e8198]'}`}
              >
                · {t.version} {dynamicVersion}
              </span>
            </div>
            <div
              className={`flex items-center gap-2 text-xs font-medium ${theme === 'light' ? 'text-green-600' : 'text-[#11bfaf]'}`}
            >
            ...
            >
              ...
              {isUnlocked ? (
                ...
                <span className="text-[#11bfaf] opacity-80">{t.statusOnline}</span>
              )}{' '}
              · {t.secLevelHigh}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-2">
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              ...
              <ChevronDown
                className={`w-3 h-3 transition-transform duration-300 ${themeExpanded ? 'rotate-180' : ''}`}
              />
            </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1 border-t border-dashed border-cyan-500/10">
                    ...
                    >
                      <div className="flex items-center gap-2 w-full mb-1">
                        <Sun
                          className={`w-4 h-4 ${theme === 'light' ? 'text-cyan-500' : 'text-[#6e8198]/30'}`}
                        />
                        <span
                          className={`text-[10px] font-bold ${theme === 'light' ? 'text-slate-800' : 'text-[#6e8198]/30'}`}
                        >
                          {t.lightMode}
                        </span>
                        <ArrowDown
                          className={`w-3 h-3 ml-auto rotate-[-135deg] ${theme === 'light' ? 'text-cyan-400' : 'text-[#173242]/40'}`}
                        />
                      </div>
                    </button>
                    ...
                    >
                      <div className="flex items-center gap-2 w-full mb-1">
                        <Moon
                          className={`w-4 h-4 ${theme === 'dark' ? 'text-[#12d8ff]' : 'text-slate-300'}`}
                        />
                        <span
                          className={`text-[10px] font-bold ${theme === 'dark' ? 'text-[#eaf4ff]' : 'text-slate-300'}`}
                        >
                          {t.darkMode}
                        </span>
                        <ArrowDown
                          className={`w-3 h-3 ml-auto rotate-[-135deg] ${theme === 'dark' ? 'text-[#12d8ff]/50' : 'text-slate-100'}`}
                        />
                      </div>
                    </button>
                  </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              ...
              <ChevronDown
                className={`w-3 h-3 transition-transform duration-300 ${languageExpanded ? 'rotate-180' : ''}`}
              />
            </div>
                        ...
                        className={`flex items-center justify-center py-1.5 px-1 rounded-lg border transition-all text-[9px] font-mono cursor-pointer hover:scale-[1.02] active:scale-[0.98] ${
                          language === lang.id
                            ...
                            : theme === 'light'
                              ...
                              : 'bg-white/[0.02] border-white/[0.05] text-[#6e8198] hover:border-cyan-800 hover:text-cyan-400'
                        }`}
                      >
                        {lang.label}
                      </button>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span
            className={`text-sm font-bold ${theme === 'light' ? 'text-cyan-700' : 'text-[#eaf4ff]'}`}
          >
            {t.emergencyAnchor}
          </span>
          <span
            className={`hidden md:inline text-xs ${theme === 'light' ? 'text-slate-400' : 'text-[#6e8198]/40'}`}
          >
            {' '}
            ——{' '}
            {passwordHash
              ...
              : t.emergencyAnchorDesc}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span
            className={`text-[10px] font-mono ${theme === 'light' ? 'text-cyan-600' : 'text-[#12d8ff]/80'}`}
          >
            {passwordHash
              ? language === 'zh'
                ? '点击检视'
          ...
          <ChevronRight
            className={`w-4 h-4 ${theme === 'light' ? 'text-cyan-300' : 'text-[#6e8198]/30'}`}
          />
        </div>
      </div>
    </div>
