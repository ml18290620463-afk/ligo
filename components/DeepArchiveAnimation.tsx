...
export const DeepArchiveAnimation: React.FC<DeepArchiveAnimationProps> = ({ theme, onComplete }) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const globalOpacity = elapsed < 500 ? elapsed / 500 : elapsed > duration - 500 ? (duration - elapsed) / 500 : 1;
      ...
      particles.forEach(p => {
        ...
        ctx.beginPath();
        const sx = (1 - startT) * (1 - startT) * p.start.x + 2 * (1 - startT) * startT * cp.x + startT * startT * p.end.x;
        const sy = (1 - startT) * (1 - startT) * p.start.y + 2 * (1 - startT) * startT * cp.y + startT * startT * p.end.y;
