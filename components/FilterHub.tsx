...
import { Search, Tag, Database, Plus, X, Folder, LayoutGrid, Trash2, Clock } from 'lucide-react';
      ...
      <div className="p-6 space-y-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className={`flex items-center gap-2 text-xs font-mono uppercase tracking-[0.2em] ${theme === 'light' ? 'text-slate-400' : isGreen ? 'text-green-700/60' : 'text-cyan-700/60'}`}>
          ...
          <div className="flex flex-wrap gap-4">
            <button
              ...
              className={`px-4 sm:px-8 py-3 text-sm font-black tracking-[0.2em] transition-all relative overflow-hidden group ${
                selectedCategory === 'all' 
                  ? (theme === 'light' ? 'bg-[#007a8c] text-white shadow-lg' : isGreen ? 'bg-green-500 text-black shadow-[0_0_20px_rgba(34,197,94,0.6)]' : 'bg-cyan-500 text-black shadow-[0_0_20px_rgba(6,182,212,0.6)]') 
                  : (theme === 'light' ? 'bg-slate-50 text-slate-400 hover:bg-slate-100 border border-slate-100' : isGreen ? 'bg-transparent border border-green-900/30 text-green-800 hover:text-green-400/60 hover:bg-green-900/10' : 'bg-transparent border border-cyan-900/30 text-cyan-800 hover:text-cyan-400/60 hover:bg-cyan-900/10')
              }`}
            >
              <div className="flex items-center gap-2">
              ...
              {selectedCategory === 'all' && <motion.div layoutId="cat-active" className="absolute bottom-0 left-0 w-full h-1 bg-white/30" />}
            </button>
            ...
            >
              ...
              {selectedCategory === 'uncategorized' && <motion.div layoutId="cat-active" className="absolute bottom-0 left-0 w-full h-1 bg-white/30" />}
            </button>
                ...
                <button
                  ...
                  className={`px-4 sm:px-8 py-3 text-sm font-black tracking-[0.2em] transition-all relative overflow-hidden pr-12 min-w-[120px] ${
                    selectedCategory === container.id 
                      ? (theme === 'light' ? 'bg-[#007a8c] text-white shadow-lg' : isGreen ? 'bg-green-500 text-black shadow-[0_0_20px_rgba(34,197,94,0.6)]' : 'bg-cyan-500 text-black shadow-[0_0_20px_rgba(6,182,212,0.6)]') 
                      : (theme === 'light' ? 'bg-slate-50 border border-slate-100 text-slate-400 hover:text-[#007a8c]' : isGreen ? 'bg-transparent border border-green-900/30 text-green-800 hover:text-green-400 hover:bg-green-900/10' : 'bg-transparent border border-cyan-900/30 text-cyan-800 hover:text-cyan-400 hover:bg-cyan-900/10')
                  }`}
                >
                  ...
                  {selectedCategory === container.id && <motion.div layoutId="cat-active" className="absolute bottom-0 left-0 w-full h-1 bg-white/30" />}
                </button>
                <button
            ...
            <AnimatePresence>
              {isAddingContainer && (
                ...
                >
                  ...
                  />
                  <button type="submit" className={`p-2 ${theme === 'light' ? 'text-cyan-600' : 'text-cyan-400'}`}>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            ] satisfies { id: GroupingMode; label: string }[]).map((mode) => {
            ...
            <input
              ...
              className={`
                ...
                ${theme === 'light' 
                  ...
                  : isGreen ? 'border-green-950/40 focus:border-green-500/50 text-green-400 placeholder:text-green-950/80' : 'border-cyan-950/40 focus:border-cyan-500/50 text-cyan-400 placeholder:text-cyan-950/80'}
              `}
            />
            {/* Search Glow Line */}
            <motion.div 
