...
test('completes onboarding and creates a journal entry', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /马斯克|Elon Musk/i }).first().click();
  await page.getByRole('button', { name: /老子|Laozi/i }).first().click();
  await page.getByRole('button', { name: /加缪|Camus/i }).first().click();
  ...
  await page.getByPlaceholder('不要急着说正确的话，说真实的...').fill('这是一条由 Playwright 创建的端到端验证记录。');
